...
test.describe('Favorites Functionality', () => {
  ...
  test('should show empty state when no favorites', async ({ page }) => {
    ...
    await expect(
      page.getByText(
        'Add Pokemon to your favorites by clicking the heart icon on any Pokemon card.',
      ),
    ).toBeVisible();
  });
  ...
  test('should add and remove favorites from Pokemon modal', async ({
    page,
  }) => {
    ...
    const modalFavoriteButton = page
      .locator('[data-testid="pokemon-modal"] button')
      .nth(1);
